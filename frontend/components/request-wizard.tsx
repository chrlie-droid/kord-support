...
type Step = 'object' | 'identity' | 'category' | 'questions' | 'chat';
...
const storageKey = 'kord-support-client-request-draft-v2';
...
export function RequestWizard() {
  ...
  useEffect(() => {
    ...
    const savedIdentity = window.localStorage.getItem('kord-support-employee-identity-v1');
  ...
  useEffect(() => {
    ...
    if (draft.employeeName) {
      window.localStorage.setItem(
        'kord-support-employee-identity-v1',
        JSON.stringify({
  ...
  function resetIdentity() {
    window.localStorage.removeItem('kord-support-employee-identity-v1');
    setDraft({ ...draft, employeeName: '', employeeRole: '', employeePhone: '', step: 'identity' });
  }
    ...
    <div className="space-y-5">
      ...
      {draft.step === 'object' ? (
        <section className="rounded-3xl border bg-white p-6 shadow-sm">
          <p className="text-sm text-slate-500">Шаг 1 из 5</p>
      ...
      {draft.step === 'identity' ? (
        <section className="rounded-3xl border bg-white p-6 shadow-sm">
          ...
          <p className="mt-4 text-sm text-slate-500">Шаг 2 из 5 · {selectedObject?.name}</p>
          <h1 className="mt-2 text-3xl font-bold tracking-tight">Представьтесь</h1>
          <p className="mt-2 text-slate-600">Нам важно точно знать, кто оставил обращение. Это не регистрация и не пароль.</p>

          {draft.employeeName ? (
            <div className="mt-5 rounded-2xl bg-emerald-50 p-4 text-sm text-emerald-800">
              Сейчас выбрано: <b>{employeeTitle}</b>{draft.employeePhone ? ` · ${draft.employeePhone}` : ''}
          ...
          <button type="button" disabled={!draft.employeeName.trim()} onClick={() => setDraft({ ...draft, step: 'category' })} className="mt-6 rounded-2xl bg-slate-950 px-5 py-3 font-semibold text-white disabled:bg-slate-300">Продолжить</button>
        </section>
      ) : null}
      ...
      {draft.step === 'category' ? (
        <section className="rounded-3xl border bg-white p-6 shadow-sm">
          ...
          <p className="mt-4 text-sm text-slate-500">Шаг 3 из 5 · {selectedObject?.name} · {employeeTitle}</p>
      ...
      {draft.step === 'questions' ? (
        <section className="rounded-3xl border bg-white p-6 shadow-sm">
          ...
          <p className="mt-4 text-sm text-slate-500">Шаг 4 из 5 · {selectedObject?.name} · {selectedCategory?.title}</p>
